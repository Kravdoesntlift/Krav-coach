...
export default function LiveWorkout({ exercises, dayId, clientId, dayLabel, onComplete, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    const onVisible = () => { if (document.visibilityState === "visible") void requestWL(); };
    void requestWL();
  ...
  useEffect(() => {
    try {
      ...
      navigator.mediaSession.metadata = new MediaMetadata({
        ...
        artist: `KRAV · ${isEN ? "Set" : "Série"} ${doneSets + 1}/${ex?.sets ?? 0}`,
        album: dayLabel,
  ...
  }, [step, setLogs, sorted, dayLabel, isEN, totalSteps]);
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data } = await supabase
        .from("workout_logs").select("exercise_name, sets, logged_at")
  ...
  function skipRest() {
    ...
    setRestActive(false); setRestLeft(0);
  }
  ...
  function toggleSet(exIdx: number, setIdx: number) {
    setSetLogs((prev) => {
      ...
      copy[exIdx][setIdx].done = !wasDone;
      if (!wasDone) {
  ...
  return (
    ...
    >
      {/* ─── REST OVERLAY ─────────────────────────────────────── */}
      {restActive && (
        <div className="absolute inset-0 z-20 bg-black flex flex-col items-center justify-center gap-7">
          ...
          <div className="relative w-56 h-56">
            <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
              ...
              <circle cx="50" cy="50" r="44" fill="none" stroke="#C9A84C" strokeWidth="5"
                strokeDasharray={`${276 * restPct} 276`} strokeLinecap="round"
                className="transition-all duration-1000" />
            </svg>
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-white font-black tabular-nums leading-none" style={{ fontSize: "5rem" }}>
                {restLeft}
              </span>
              <span className="text-zinc-500 text-xs mt-2">
                {isEN ? "seconds" : "segundos"}
              </span>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            {[30, 60, 90, 120].map((s) => (
              <button key={s}
                onClick={() => { setRestDur(s); setRestLeft(s); }}
                className="w-14 py-2.5 rounded-2xl text-sm font-bold transition-all active:scale-95"
                style={restDur === s
          ...
          <button onClick={skipRest}
            className="px-8 py-3 rounded-2xl border border-zinc-800 bg-zinc-900 text-white text-sm font-semibold active:scale-95">
            {isEN ? "Skip →" : "Saltar →"}
          </button>
        </div>
      )}
      ...
      {/* ─── FINISH SCREEN ────────────────────────────────────── */}
      {finishing && (
        <div className="absolute inset-0 z-20 bg-black overflow-y-auto flex flex-col px-5 gap-5"
          ...
          <div>
            <p className="text-zinc-500 text-xs font-semibold tracking-widest uppercase mb-3">
              {isEN ? "How did it go?" : "Como correu?"}
            </p>
      ...
      <div className="flex items-center gap-2 px-4 py-3">
        <button onClick={onClose}
          className="w-9 h-9 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-400 shrink-0 active:scale-90">
          ✕
        </button>
          ...
          {sorted.map((_, i) => {
            ...
            return (
              <button key={i} onClick={() => setStep(i)}
                className="shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-black transition-all active:scale-90"
                style={active
                  ? { background: "linear-gradient(135deg,#E8C96B,#A8893A)", color: "#000" }
                  : done
      ...
      {/* ─── PROGRESS BAR ─────────────────────────────────────── */}
      <div className="h-px bg-zinc-900 mx-4 mb-4">
      ...
      <div className="px-4 mb-4">
        ...
        <h2 className="text-white font-black leading-tight" style={{ fontSize: "clamp(1.4rem,6vw,2rem)" }}>
          {cur?.name}
        </h2>
        <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-1">
          <span className="text-brand-gold text-sm font-semibold">
            {cur?.sets} × {cur?.reps} {isEN ? "reps" : "reps"}
          </span>
          {prev && daysAgo !== null && (
        ...
        {cur?.notes && <p className="text-zinc-600 text-xs mt-1 italic">{cur.notes}</p>}
      </div>

      {/* ─── SET TABLE ────────────────────────────────────────── */}
      <div className="px-4 flex-1 flex flex-col min-h-0">
        {/* Column headers */}
        <div className="grid mb-1.5 px-1" style={{ gridTemplateColumns: "1.5rem 1fr 1fr 2.5rem", gap: "0.5rem" }}>
          ...
          <span className="text-zinc-700 text-[10px] font-semibold uppercase">{isEN ? "Reps" : "Reps"}</span>
          <span />
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-1.5">
          {curLogs.map((s, si) => (
            <div key={si}
              className="grid items-center rounded-2xl px-2 py-2 transition-all"
              style={{
                gridTemplateColumns: "1.5rem 1fr 1fr 2.5rem",
                gap: "0.5rem",
            ...
            >
              <span className="text-xs font-black text-center"
                style={{ color: s.done ? "#C9A84C" : "#52525b" }}>
                {si + 1}
              </span>
              <input
                type="number" inputMode="decimal" placeholder="—" value={s.weight}
                onChange={(e) => updateLog(step, si, "weight", e.target.value)}
                className="bg-zinc-800 border border-zinc-700/40 rounded-xl px-2 py-2 text-white text-sm text-center w-full focus:outline-none focus:border-brand-gold/40"
              />
              <input
              ...
              <button onClick={() => toggleSet(step, si)}
                className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-black transition-all active:scale-90"
                style={s.done
                  ? { background: "linear-gradient(135deg,#E8C96B,#A8893A)", color: "#000" }
      ...
      {/* ─── BOTTOM ACTIONS ───────────────────────────────────── */}
      <div className="px-4 pt-3 pb-3 space-y-2" style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }}>
        {/* Rest presets */}
        <div className="flex items-center gap-1.5">
          <span className="text-zinc-600 text-xs shrink-0">⏱</span>
          {[30, 60, 90, 120].map((s) => (
            <button key={s}
              onClick={() => { setRestDur(s); startRest(s); }}
        ...
        <div className="flex gap-2">
          <button onClick={() => setStep((s) => s - 1)} disabled={step === 0}
            className="px-4 py-3.5 rounded-2xl text-sm font-bold transition-all active:scale-95 disabled:opacity-20"
