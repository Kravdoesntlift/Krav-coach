...
export default function BarcodeScanner({ onFound, onClose }: Props) {
  ...
  const [message, setMessage]   = useState("A aceder à câmara…");
  ...
  const lookup = useCallback(async (code: string) => {
    ...
    try {
      ...
      setState("error");
      setMessage("Erro de rede. Verifica a ligação.");
      foundRef.current = false;
    }
  }, [onFound, stopScanner]);
  ...
  const startScanner = useCallback(async () => {
    ...
    try {
      ...
      if (/permission|denied|not allowed/i.test(msg)) {
        setState("error");
        setMessage("Acesso à câmara negado. Permite nas definições do browser e tenta de novo.");
      } else {
        setState("error");
        setMessage("Não foi possível aceder à câmara. Usa a opção de galeria em baixo.");
      }
    }
  }, [lookup, stopScanner]);
  ...
  const handleFile = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      try {
        ...
        URL.revokeObjectURL(url);
        setState("notfound");
        setMessage("Código não reconhecido na imagem. Tenta com melhor iluminação ou insere o número manualmente.");
        foundRef.current = false;
      }
    } catch {
      setState("error");
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black flex flex-col" style={{ touchAction: "none" }}>
      ...
      <div className="absolute top-0 inset-x-0 z-20 flex items-center justify-between px-5 pt-14 pb-6"
        ...
        <p className="text-white font-bold text-sm tracking-wide">Código de barras</p>
        <div className="w-16" />
      </div>
        ...
        {isScanning && (
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            ...
            <p className="text-white/65 text-xs font-medium mt-6 tracking-wide text-center px-8">
              Centra o código de barras no rectângulo dourado
            </p>
          </div>
        )}
        ...
        {state === "loading" && (
          <div className="absolute inset-0 bg-black/85 flex flex-col items-center justify-center gap-5">
            ...
            <div className="text-center space-y-1">
              ...
              <p className="text-zinc-500 text-xs">Open Food Facts · 3M+ produtos</p>
            </div>
          </div>
        )}
        ...
        {state === "requesting" && (
          <div className="absolute inset-0 bg-black flex flex-col items-center justify-center gap-6 px-8">
            ...
            <div className="text-center space-y-2">
              <p className="text-white font-bold text-base">A iniciar câmara</p>
          ...
          <div className="absolute inset-0 bg-black/92 flex flex-col items-center justify-center gap-6 px-7">
            ...
            <div className="text-center space-y-2 max-w-xs">
              <p className="text-white font-bold text-base">
                {state === "notfound" ? "Produto não encontrado" : "Câmara indisponível"}
              </p>
            ...
            {showManual ? (
              <div className="w-full space-y-3 max-w-xs">
                <div className="space-y-1.5">
                  <p className="text-zinc-500 text-xs text-center">Código EAN-13 (13 dígitos)</p>
                  <input
                    type="tel"
                    inputMode="numeric"
      ...
      {isScanning && (
        <div className="shrink-0 px-5 pb-12 pt-4 space-y-3"
          ...
          <label className="flex items-center justify-center gap-2.5 w-full py-3.5 rounded-2xl cursor-pointer transition-all active:scale-[0.98]"
            ...
            <span className="text-zinc-200 text-sm font-semibold">Usar galeria / câmara</span>
            <input ref={fileRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={handleFile} />
          </label>
      ...
      {isScanning && showManual && (
        <div className="absolute inset-x-0 bottom-0 z-30 px-5 pb-10 pt-6 space-y-3 rounded-t-3xl"
          ...
          <p className="text-white font-bold text-sm mb-3">Inserir código de barras</p>
          <div className="space-y-1.5">
            <p className="text-zinc-500 text-xs">Código EAN-13 ou EAN-8 (sem espaços)</p>
            <input
              autoFocus
              type="tel"
              inputMode="numeric"
